...
:::demo Bind `v-model` to a `Boolean` typed variable. The `--xn-switch-on-color` and `--xn-switch-off-color` CSS variables decides the background color in two states.
...
Use the `active-icon` and `inactive-icon` attribute to add icon. You can pass either string for the component name (registered in advance) or the component itself which is a SVG Vue component. Xianniu Plus has provided a set of icon that you can find at [icon](/en-US/component/icon)
...
| active-color ^(deprecated)    | background color when in `on` state ( use CSS var `--xn-switch-on-color` instead )                                                              | ^[string]                                        | ''      |
| inactive-color ^(deprecated)  | background color when in `off` state ( use CSS var `--xn-switch-off-color` instead )                                                            | ^[string]                                        | ''      |
| border-color ^(deprecated)    | border color of the switch ( use CSS var `--xn-switch-border-color` instead )                                                                   | ^[string]                                        | ''      |
