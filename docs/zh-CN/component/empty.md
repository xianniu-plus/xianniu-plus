...
您可以为 empty 组件设置自定义样式。 使用 `css/scss` 语言来更改全局或局部颜色。 我们设置了一些全局颜色变量：`--xn-empty-fill-color-0`、`--xn-empty-fill-color-1`、`--xn-empty-fill-color-2`、……、`--xn-empty-fill-color-9`。 您可以使用类似 `:root { --xn-empty-fill-color-0: red; --xn-empty-fill-color-1: blue; }` 等变量。 但通常，如果你想要更改样式，你需要更改所有颜色，因为这些颜色是一个组合。
