...
import { Edit } from '@xianniu-plus/icons-vue'
...
describe('Steps.vue', () => {
  test('render', () => {
    ...
    expect(wrapper.findAll('.xn-step').length).toBe(3)
    expect(wrapper.classes()).toContain('el-steps--horizontal')
    expect(wrapper.find('.xn-step').classes()).toContain('is-horizontal')
  })
  ...
  test('space', () => {
    ...
    expect(wrapper.find('.xn-step').attributes('style')).toMatch(
      'flex-basis: 100px;'
    )
  })
  ...
  test('alignCenter', () => {
    ...
    expect(wrapper.find('.xn-step').classes()).toContain('is-center')
  })
  ...
  test('direction', () => {
    ...
    expect(wrapper.find('.xn-step').classes()).toContain('is-vertical')
  })
  ...
  test('active', async () => {
    ...
    await nextTick()
    expect(
      wrapper.findAll('.xn-step')[0].find('.xn-step__head').classes()
    ).toContain('is-process')
    expect(
      wrapper.findAll('.xn-step')[1].find('.xn-step__head').classes()
    ).toContain('is-wait')
    expect(
      wrapper.findAll('.xn-step')[2].find('.xn-step__head').classes()
    ).toContain('is-wait')
  ...
  test('step attribute', () => {
    ...
    expect(wrapper.find('.xn-step__head').classes()).toContain('is-wait')
    expect(wrapper.find('.xn-step__title').text()).toBe('title')
    expect(wrapper.find('.xn-step__description').text()).toBe('description')
  ...
  test('step slot', () => {
    ...
    expect(wrapper.find('.xn-step__title').text()).toBe('A')
    expect(wrapper.find('.xn-step__description').text()).toBe('B')
  })
  ...
  test('order of step', async () => {
    ...
    data.value = ['a', 'b', 'c']
    await nextTick()
    wrapper.findAll('.xn-step__icon-inner').forEach((domWrapper, index) => {
