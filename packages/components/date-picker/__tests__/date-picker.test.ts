...
import { CommonPicker } from '@xianniu-plus/components/time-picker'
import Input from '@xianniu-plus/components/input'
import zhCn from '@xianniu-plus/locale/lang/zh-cn'
import enUs from '@xianniu-plus/locale/lang/en'
...
describe('DatePicker', () => {
  ...
  describe('value-format', () => {
    it('with literal string', async () => {
      ...
      const valueFormat = '[Xianniu-Plus] DD/MM YYYY'
      ...
      `,
        ...
        },
        {
          methods: {
            changeValue() {
              this.value = '[Xianniu-Plus] 31/05 2021'
            },
          },
        }
      )
      ...
      expect(vm.value).toBe(
        dayjs(
          `[Xianniu-Plus] 01/${`0${day.month() + 1}`.slice(-2)} ${day.year()}`,
          valueFormat
        ).format(valueFormat)
      )
...
describe('MonthPicker', () => {
  ...
  it('value-format', async () => {
    const valueFormat = '[Xianniu-Plus] YYYY.MM'
    const wrapper = _mount(
      `
      <el-date-picker
        type="month"
...
describe('YearPicker', () => {
  ...
  it('value-format', async () => {
    const valueFormat = '[Xianniu-Plus] YYYY'
    const wrapper = _mount(
      `
      <el-date-picker
        type="year"
