...
const assertCurrent = (wrapper: VueWrapper<any>, page: number) => {
  expect(wrapper.find('.xn-pager li.is-active.number').text()).toBe(
    String(page)
  )
}
const assertPages = (wrapper: VueWrapper<any>, total: number) => {
  expect(wrapper.find('.xn-pagination .xn-pager li:last-child').text()).toBe(
    String(total)
  )
}
...
describe('Pagination', () => {
  describe('test invalid usages', () => {
    ...
    test('both absence of total & pageCount is invalid', async () => {
      ...
      expect(wrapper.find('.xn-pagination').exists()).toBe(false)
      expect(console.warn).toHaveBeenCalled()
      total.value = 100
      await nextTick()
      expect(wrapper.find('.xn-pagination').exists()).toBe(true)
    })
  ...
  describe('test layout & layout reactive change', () => {
    ...
    const layoutSelectorPairs = [
      ['sizes', '.xn-pagination__sizes'],
      ['prev', 'button.btn-prev'],
      ['pager', 'ul.xn-pager'],
      ['next', 'button.btn-next'],
      ['jumper', '.xn-pagination__jump'],
      ['total', '.xn-pagination__total'],
    ]
    ...
    test(`layout with '->, total'`, async () => {
      ...
      assertElementsExistence(
        wrapper,
        ['.xn-pagination__total', '.xn-pagination__rightwrapper'],
        true
      )
    })
  ...
  describe('test pageSize & currentPage reactive change', () => {
    ...
    test('test currentPage change', async () => {
      ...
      await wrapper.find('.xn-pager li:last-child').trigger('click')
    ...
    test('test listener work', async () => {
      ...
      expect(count).toBe(1)
      await wrapper.find('.xn-select').trigger('click')
      await wrapper
  ...
  describe('test a11y supports', () => {
    test('test a11y attributes', async () => {
      ...
      const prev = wrapper.find('.xn-pagination .btn-prev')
      const next = wrapper.find('.xn-pagination .btn-next')
      const pagers = wrapper.findAll('.xn-pagination .xn-pager .number')
    ...
    test('test tabindex interactive', async () => {
      const wrapper = mount(() => <Pagination total={100} />)
      await wrapper.find('.xn-pager li:nth-child(2)').trigger('click')
      assertCurrent(wrapper, 2)
      await wrapper.find('.xn-pager li:nth-child(3)').trigger('click', {
        key: 'Enter',
      })
      assertCurrent(wrapper, 3)
      // TODO getComputedStyle is not implemented in jsdom, so I duno how to assert style of psuedo-class
      /*
       * await wrapper.find('.xn-pager li:nth-child(3)').trigger('keyup', {
       *   key: 'Tab',
       * })
       * const style = window.getComputedStyle(wrapper.find('.xn-pager li:nth-child(4)').element, ':focus-visible')
    ...
    test('test tabindex disabled', async () => {
      ...
      expect(
        wrapper.find('.xn-pager li:first-child').attributes('tabindex')
      ).toBe('-1')
