...
import { CaretTop } from '@xianniu-plus/icons-vue'
...
describe('Backtop.vue', () => {
  test('render', async () => {
    ...
    expect(wrapper.find('.xn-backtop').exists()).toBe(false)
    wrapper.element.scrollTop = 2000
    await wrapper.trigger('scroll')
    expect(wrapper.find('.xn-backtop').exists()).toBe(true)

    expect(wrapper.find('.xn-backtop').attributes('style')).toBe(
